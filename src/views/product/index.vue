<template>
  <div class="app-container">
    <el-row type="flex" class="row-bg" justify="end">
      <el-button type="primary" justify="end" @click="handleAddPProduct">新增產品</el-button>
    </el-row>
    ...
    <el-table
      ...
      :gutter="10"
    >
      <el-table-column label="產品名稱" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.product_name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="售價" width="150" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.amount }}</span>
        </template>
      </el-table-column>
      <el-table-column label="時間" width="550" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.minute }}</span>
        </template>
      </el-table-column>
      <el-table-column label="內容" width="150" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column class-name="status-col" label="狀態" width="100" align="center">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status | statusFilter">{{ scope.row.status === 0 ? '已停售':'販售中' }}</el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      performance: Object.assign({}, defaultProduct),
      list: null,
      listLoading: true,
      dialogVisible: false,
      dialogType: 'new'
  ...
  methods: {
    ...
    handleAddPProduct() {
      this.performance = Object.assign({}, defaultProduct)
      this.dialogType = 'new'
