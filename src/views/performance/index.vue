  ...
  <div class="app-container">
    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'修改訂單':'新增訂單'">
      <el-form :model="performance" label-width="80px" label-position="left">
        <el-form-item label="按摩師傅">
          <el-select value-key="id" v-model="selectCommunityData" filterable placeholder="請選擇按摩師傅">
      ...
      <el-form :model="performance" label-width="80px" label-position="left">
        <el-form-item label="消費商品">
          <el-select value-key="value" v-model="value1" multiple placeholder="请选择">
            <el-option
  ...
  methods: {
    ...
    handleCommand(type) {
      getMasterData().then(response => {
        this.masterList = response.data.items
      })
    }
  }
}
